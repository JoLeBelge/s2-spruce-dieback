...
int ArgumentHandler::processArguments() {
  ...
  // short options
  char const *short_options = "A:C:D:F:HM:NOP:Q:R:S:U:XZa:b:c:d:f:hil::m:o:pr:s:t:uvwy:z:e:";
  ...
  // long options: longname, no/optional/required argument?, flag(not used!), shortname
    const struct option long_options[] = {
      ...
      { "relPhyto",             required_argument,  0, 'e'},
  ...
  while (1) {
    ...
    case 'e':
      //std::cout << "parse l'option --relPhyto " << std::endl;
     //std::cout << optarg << std::endl;
      splitString(mVCodeEsp, optarg, ',');
