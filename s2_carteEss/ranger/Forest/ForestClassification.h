...
public:
  ...
  std::map<int,double> getPredictions(){return mPredictions;}
  int getClassMaj(){
      double aMaj(0.0);
      int aClassMaj(0);
      for (auto kv :mPredictions){
       if (kv.second>aMaj)  {aClassMaj=kv.first; aMaj=kv.second;}
      }
      return aClassMaj;
  }
...
protected:
  ...
  std::map<int,double> mPredictions;
