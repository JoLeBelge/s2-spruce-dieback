...
void ForestClassification::writePredictionFile() {
  ...
  // std::cout << "results ---\n"<< std::endl;
  if (predict_all) {
      // ici je change le résultats retourné par RF phyto
      std::map<double,double> aSumVote;
      for (int i(0) ; i<class_values.size(); i++){
          //std::cout << "i = "<< i << " , " << class_values[i]<< " getclass value " << getClassValues()[i] << std::endl;
    ...
    for (size_t k = 0; k < num_trees; ++k) {
      ...
      for (size_t i = 0; i < predictions.size(); ++i) {
        for (size_t j = 0; j < predictions[i].size(); ++j) {
          ...
          aSumVote.at(predictions[i][j][k]) +=1;
        }
      }
      //outfile << std::endl;
    }
