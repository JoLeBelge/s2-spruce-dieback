...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry,
    ...
    PredictionType prediction_type, uint num_random_splits, uint max_depth, const std::vector<double> &aVMetrics, const std::string &aHeader) {
  ...
  // ici je fait une condition du genre "si argument mVCodeEss not null"
  if (aVMetrics.size()>0){

  init(dependent_variable_name, memory_mode,load_data_from_vMetrics(aVMetrics,aHeader), mtry,
  ...
  if (!case_weights_file.empty()) {
    ...
    if (case_weights.size() != num_samples) {
		std::cout << "num_samples " << num_samples << ", case weights " << case_weights.size() << std::endl;
