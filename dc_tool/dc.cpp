...
void dc::genClassRaster(GDALDataset *DShouppiers, GDALDataset *DSzone, std::string aOut){
    ...
    std::string aCommand(std::string("gdal raster calc -i 'A=")+out+"' --calc '"+exp+"' --ot Int16 --co 'COMPRESS=DEFLATE' a_nodata 255 --overwrite -o "+ reclassFile);
...
void dc::modifyClassRaster(std::string aOut, std::string exp){
    ...
    snprintf(out, NPOW_10, "%s/X%04d_Y%04d/%s.tif", phl->d_mask, tileX, tileY, aOut.c_str());
